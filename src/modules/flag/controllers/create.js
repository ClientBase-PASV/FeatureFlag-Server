...
export default async function flagCreate(req, res) {
  ...
  const addFragToGroupResult = await addFlagToGroupQuery({
    flagId: _id,
    groupId,
  });

  console.log(createFlagQueryResult, addFragToGroupResult);

  if (createFlagQueryResult.success && addFragToGroupResult.success) {
